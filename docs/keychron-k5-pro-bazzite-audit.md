...
| Open Files | Enabled | `fn+F4` emits `Super+E`; the extension can also open the default file manager with `gio open ~`. |
...
| Other keyboard backlight and RGB items | Disabled reference | The Keychron handles these in firmware; no working OS backlight device is exposed. |
...
- `fn+F5/F6` appear to change keyboard backlight patterns/effects on this K5 Pro, rather than exposing OS-controllable brightness.
- Treat the remaining Keychron backlight/RGB controls as firmware-only shortcuts unless a VIA command is added for them.
